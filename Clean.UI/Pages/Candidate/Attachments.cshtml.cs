...
{
    public class AttachmentsModel : BasePage
    {
        ...
        public async Task OnGetAsync([FromRoute] int id)
        {
            ...
            var documentTypes = await Mediator.Send(new GetDocumentTypeQuery() { Catagory = "HMIS"  });
        ...
        public async Task<IActionResult> OnPostSave([FromBody] AttachmentCommand command)
        {
        ...
        public async Task<IActionResult> OnPostSearch([FromBody] SearchAttachmentQuery query)
        {
            query.ScreenId = 8;
            try
            {
