...
(function () {
    ...
    clean.Tazkira.prototype = {
        ...
        init: function () {
            ...
            control = "<input id='$id' name='$name' class='form-control alt-tazkira' type='number' required/>";
            var input = self.template.replace('$Control', control);

            // Typable Inputs For Page Number and Registeration Number
            var pg = input.replace('$width', '3').replace('$Label', 'نمبر صفحه').replace('$id', 'pg').replace('$g-class', 'old').replace('$name', 'Page');
            self.row.append(pg);
            self.page = self.row.find('#pg');

            var no = input.replace('$width', '3').replace('$Label', 'نمبر ثبت').replace('$id', 'no').replace('$g-class', 'old').replace('$name', 'No');
            self.row.append(no);
            self.No = self.row.find('#no');
            var sn = input.replace('$width', '3').replace('$Label', 'نمبر صکوک').replace('$id', 'sn').replace('$g-class', 'always').replace('$name', 'SerialNumber');
            self.row.append(sn);
        ...
        changeselect: function (v) {
            ...
            if (v == 'تذکره ورقی') {
                ...
                self.row.find('.old').each(function (i, item) {
                    $(item).find('select,input').removeAttr('disabled');
                });
                self.row.find('.alt-tazkira').val("");
            }
            else {
                self.row.find('.old').each(function (i, item) {
                    $(item).find('select,input').attr('disabled', 'true');
                });
                self.row.find('.old').hide();
            }
