...
(function () {
    clean.form = function (opt) {
        ...
        this.chart = this.el.find('.form-chart');
    ...
    clean.form.prototype = {
        ...
        getfields: function () {
            ...
            var filtered = self.fields.filter(function (item, index, arry) {
                if (($(self.fields[item]).attr('id') || "").startsWith(self.prefix))
            ...
            if (!self.el.hasClass('sub-form')) {
                ...
                var f = $(self.parent.fields.last()).attr('id');
                fieldcounter = ($('#' + f).attr("tabindex") - 1) + 2;

            }

            $.each(self.fields, function (index, field) {
                $(field).attr("tabindex", "");
                $(field).attr("tabindex", fieldcounter);
        ...
        loaddatepicker: function () {
            ...
            self.el.find('.Shamsi').each(function () {
                ...
                var shamsi = el.MdPersianDateTimePicker({
                    ...
                    disableAfterToday: el.hasClass('no-future'),
                    isGregorian: false,
        ...
        construct: function () {
            ...
            self.el.find('.discount-id').each(function () {
                var opt = {};
                opt.el = $(this).find('.discount-id-input');
                opt.parent = $(this).parent();
                opt.form = self;
                self.discount = new clean.Discount(opt);
            });
            ...
            if (!$.isEmptyObject(self.chart))
                self.el.parents('.panel').append(self.chart);
        ...
        save: function () {
            ...
            self.getfields();
            var _discount = false;
            if (!$.isEmptyObject(self.discount)) {
                _discount = self.discount.validate();
                if (_discount) {
                    self.discount.el.val(self.discount.val());
                }
            }
            else {
                _discount = true;
        ...
        report: function () {
            var self = this;
            if (self.el.valid()) {
                var path = self.path + '/report';
                var data = {};
                self.fields.each(function () {
                    var fld = $(this);
                    var col = fld.attr('id').substring(self.prefix.length);
                    Object.defineProperty(data, col.toString(), { value: fld.is(':checkbox') ? fld.is(':checked') : fld.val().toString(), enumerable: true });
                });
                ...
                clean.data.post({
                    ...
                    success: function (msg) {
                        if (msg.status > 0) {
                            var list = msg.data.list;
                            var options = {};
                            options.el = self.chart;
                            options.data = list;
                            options.legend = [];
                            options.date = [];
                            let listCount = 0;
                            $.each(list, function (i, k) {
                                options.legend.push(k.name);
                                listCount++;
                            });

                            // date=data.date.filter(data.date);
                            date = [...new Set(list.map(x => x.date))];



                            if (listCount > 0) {
                                new clean['chart'](options);
                            } else {
                                $('#GraphDIV').html(' <div class="chart-container " ><div class="chart has-fixed-height has-minimum-width" id="Chart_ROSE" chart-type="Chart_ROSE">Empty</div></div>');
                            }
                        }
                        else {
                            clean.widget.warn(msg.text, msg.description);
